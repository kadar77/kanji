...
2. **Translate** each batch with Codex, fed *both* the English gloss and its Japanese context. Rules the model followed: use Japanese only to disambiguate; translate into natural Mongolian **Cyrillic**; preserve `;`/`,` multi-sense separators; keep genuine proper nouns / scientific binomials (e.g. *Solanum melongena*, iPhone) untranslated; dictionary-style glosses, no romaji or notes. (This was run as an in-session Codex multi-agent workflow — ~260 agents, one per batch.)
